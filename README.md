...
이 시스템은 Obsidian에 정리된 기술 관련 Markdown 파일을 Google Drive에 업로드한 후, 파일을 스캔하고 편집하여 WordPress 블로그에 자동으로 업로드합니다. 스프링 배치와 API를 통해 블로그 글을 주기적으로 관리하고, 이미지를 포함한 파일들을 WordPress에 업로드하여 손쉽게 블로그를 업데이트할 수 있도록 지원합니다.
